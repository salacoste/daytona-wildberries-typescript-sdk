#!/usr/bin/env node
/**
 * Split oversized shards into smaller files
 */
...
class ShardSplitter {
    ...
    splitShardByEndpoints(shardPath) {
        console.log(`\nSplitting oversized shard: ${path.basename(shardPath)}`);
        ...
        // Split into chunks of 2 endpoints each for aggressive size control
        const chunkSize = 2;
        const chunks = [];
        ...
        chunks.forEach((chunk, index) => {
            const chunkSpec = {
                ...
                info: {
                    ...spec.info,
                    title: `${spec.info.title} (Part ${index + 1}/${chunks.length})`
                },
            ...
            // Add endpoints to this chunk
            chunk.forEach(endpoint => {
                if (!chunkSpec.paths[endpoint.path]) {
                    chunkSpec.paths[endpoint.path] = {};
                }
                chunkSpec.paths[endpoint.path][endpoint.method] = endpoint.spec;
            });

            const chunkFile = path.join(moduleDir, `${baseName}-part${index + 1}.yaml`);
            this.saveYaml(chunkSpec, chunkFile);

            const lineCount = this.countLines(chunkFile);
            console.log(`  → Part ${index + 1}: ${chunk.length} endpoints, ${lineCount} lines`);
            chunkFiles.push(chunkFile);
        });

        // Remove original oversized shard
        fs.unlinkSync(shardPath);
        console.log(`✓ Removed original oversized shard: ${path.basename(shardPath)}`);
    ...
    processModule(moduleName) {
        ...
        if (oversized.length > 0) {
            ...
            oversized.forEach(filepath => {
                this.splitShardByEndpoints(filepath);
            });
...
function main() {
    ...
    const splitter = new ShardSplitter(baseDir);
    ...
    // Verify all shards are now under limit
    console.log('\nVerifying all shards are under 1000 lines:');
    let allValid = true;
    ...
    if (allValid) {
        console.log('\n✓ All shards successfully validated!');
    } else {
        console.log('\n⚠ Some shards still exceed 1000 lines and may need manual splitting');
    }
}

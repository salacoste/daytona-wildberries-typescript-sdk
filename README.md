...
## What's New (v4.2.0–v4.4.0) — August–September 2026

🚀 **September 2026 WB API sync — 15 news items implemented** (v4.3.0 + v4.4.0):

- ⚠️ **`getAdvBudget()` deprecated** — WB disables `GET /adv/v1/budget` on **2026-11-16**. Migrate to `postV2Budget()` (up to 50 campaigns per request). Removal in v5.
- 🆕 **Real-time Order Feed** — `sdk.analytics.getOrderFeed()` replaces supplier orders/sales reports (unified orders+buyouts, statuses + cancel reasons, B2B flag, `snapshotTime`-cursor pagination).
- 🆕 **Final prices** — `sdk.ordersDBS.getOrdersFinalPrice()` / `sdk.inStorePickup.getOrdersFinalPrice()`: buyer sums with all discounts and cashback (`originalFinalPrice` — use for calculations).
...
📖 Full guide: **[WB API Changes — September 2026](./docs/guides/wb-api-changes-2026-09.md)** · 306 public methods · 2,462 tests.
...
**Current Status:** 🟢 Production Ready | **SDK Version:** 4.4.0
...
## Что нового (v4.2.0–v4.4.0) — август–сентябрь 2026

🚀 **Синхронизация WB API сентября 2026 — внедрено 15 новостей** (v4.3.0 + v4.4.0):

- ⚠️ **`getAdvBudget()` устарел** — WB отключает `GET /adv/v1/budget` **16.11.2026**. Мигрируйте на `postV2Budget()` (до 50 кампаний за запрос). Удаление в v5.
- 🆕 **Лента заказов в реальном времени** — `sdk.analytics.getOrderFeed()` заменяет отчёты supplier orders/sales (заказы+выкупы одним потоком, статусы и причины отмен, признак B2B, пагинация курсором `snapshotTime`).
- 🆕 **Финальные цены** — `sdk.ordersDBS.getOrdersFinalPrice()` / `sdk.inStorePickup.getOrdersFinalPrice()`: суммы к оплате покупателем со всеми скидками и кэшбеком (`originalFinalPrice` — использовать для расчётов).
...
📖 Полный гид: **[Изменения WB API — сентябрь 2026](./docs/guides/wb-api-changes-2026-09.md)** · 306 публичных методов · 2 462 теста.
...
**Текущий Статус:** 🟢 Production Ready | **Версия SDK:** 4.4.0

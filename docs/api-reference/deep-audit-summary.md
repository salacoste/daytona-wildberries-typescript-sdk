...
| ordersFBS | task-175 | ⚠️ 1 CRIT | archive endpoint gap; 2 HIGH (crossborder status, dead type) |
...
| ordersDBS | task-173 | 🔴 5 CRIT | `DBSOrderMeta` number↔string; **`DeliveryDateInfo` invented**; `getGroupsInfo`/`getDeliveryDates` wrong wire shape |
...
**Totals:** 3 CLEAN · 9 with findings · ~45 findings (many CRITICAL / runtime-breaking).

...
> Охватывает SDK **v4.3.0** – **v4.6.0**: шестнадцать новостей Wildberries API,
...
| **2026-10-26** | WB **отключает** `GET /api/v1/analytics/goods-return` | Мигрируйте `reports.getAnalyticsGoodsReturn()` → `getAnalyticsV1GoodsReturn()` (deprecated с v4.6.0, удаление в v5) |
...
### Отчёт по возвратам v1 (v4.6.0)

- **`sdk.reports.getAnalyticsV1GoodsReturn(params)`** — новая версия отчёта
  «Возврат и перемещение товаров». Все пять параметров обязательны
  (`dateFrom`, `dateTo`, `status`, `limit`, `offset`); фильтр `status`
  (`active` / `archive`) заменяет поле `isStatusActive`, в ответе появился
  общий счётчик `count` за весь период. Окно — максимум **31 день**;
  пустой результат — HTTP 204. Обратите внимание на порядок сегментов пути —
  `/api/analytics/v1/item-returns`: операция, которую текст новости WB назвал
  «goods-return», в официальной спеке смонтирована на `item-returns`.
- Переименования полей: `barcode`→`sku`, `status`→`returnStatus`,
  `reason`→`returnReason` (только для неопознанных возвратов); новые поля
  `kiz` (Честный знак), `subjectName`, `techSize`.
- **`getAnalyticsGoodsReturn()` помечен deprecated** (WB отключает эндпоинт
  **2026-10-26**, удаление в v5) — метод теперь выдаёт однократное

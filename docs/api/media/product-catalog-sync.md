...
async function getCategories() {
  try {
    ...
    const parentCategories = await sdk.products.getParentCategories();
    ...
    if (electronicsId) {
      const subcategories = await sdk.products.getSubcategories(electronicsId);
    ...
    const characteristics = await sdk.products.getCategoryCharacteristics(categoryId);
...
async function uploadProductMedia(productId: string) {
  try {
    ...
    for (const imagePath of imagePaths) {
      ...
      const result = await sdk.products.uploadMedia({
        productId: productId,
        file: imageBuffer,
...
async function updatePricing(productId: string) {
  try {
    ...
    const result = await sdk.products.updatePrice({
      productId: productId,
...
async function syncProductCatalog() {
  try {
    ...
    await sdk.products.updatePrice({
      productId: product.data.id,
      price: 54999,
      discount: 15
    });

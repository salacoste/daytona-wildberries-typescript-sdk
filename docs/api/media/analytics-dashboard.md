...
async function generateCSVReport() {
  try {
    ...
    const reportRequest = await sdk.analytics.generateReport({
      reportType: 'sales',
    ...
    while (attempts < maxAttempts) {
      ...
      reportStatus = await sdk.analytics.getReportStatus(reportRequest.data.reportId);
    ...
    const reportData = await sdk.analytics.downloadReport(reportRequest.data.reportId);

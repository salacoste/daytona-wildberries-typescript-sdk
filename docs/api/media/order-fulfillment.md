...
async function getOrderDetails(orderId: string) {
  try {
    const order = await sdk.ordersFBS.getOrderById(orderId);
...
async function fulfillOrders() {
  try {
    ...
    for (const order of orders.data) {
      ...
      const details = await sdk.ordersFBS.getOrderById(order.orderId);
      console.log(`✓ Customer: ${details.data.customerInfo.name}`);
      console.log(`✓ Items: ${details.data.items.length}`);
      console.log(`✓ Total: ${details.data.totalAmount} RUB`);
...
async function updateOrderWithLogging(orderId: string, newStatus: string) {
  ...
  try {
    const result = await sdk.ordersFBS.updateOrderStatus({
      orderId,
      status: newStatus
    });

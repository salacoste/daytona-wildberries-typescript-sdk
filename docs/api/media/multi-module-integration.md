...
async function createProductWithInventory() {
  try {
    ...
    await sdk.products.publishProduct(product.data.id);
...
async function handleCrossModuleTransaction() {
  ...
  try {
    ...
    const order = await sdk.ordersFBS.updateOrderStatus(orderData);
    rollbackActions.push(() => sdk.ordersFBS.updateOrderStatus({ orderId: order.data.id, status: 'cancelled' }));
